...
function Section() {
  ...
  return (
    <div className='flex gap-10 mb-10 justify-center w-full items-center'>
      <Link 
        href="/journal" 
        className={`p-2 capitalize rounded ${pathname && !pathname.includes("timeline") && !pathname.includes("create") ? 'bg-muted' : ''}`}
      >
        Journal
      </Link>
      |
      <Link 
        ...
        className={`p-2 capitalize rounded ${pathname && pathname.includes("timeline") ? 'bg-muted' : ''}`}
      >
        Timeline
      </Link>
      |
      <Link 
        href="/journal/create" 
        className={`p-2 capitalize rounded ${pathname && pathname.includes("create") ? 'bg-muted' : ''}`}
      >
        Create
      </Link>
    </div>
